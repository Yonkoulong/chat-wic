...
} from "./UploadFileThreadPreviewModal.styles";
...
export const ModalUploadFileThreadPreview = ({
  open,
  onClose,
  data,
  formFile,
  ...
  const { pushThreadMessage, quoteThreadMessage, selectedThreadMessage } =
    ...
    try {
      ...
      if (formFile?.typeRoom === enumTypeRooms.CHANNEL) {
        let newPayloadMessageChannel = {
          ...
          replyId: quoteThreadMessage || null,
          threadId: threadId
        };
        ...
        const respPostMessage = await postMessageThread(
          threadId,
          newPayloadMessageChannel
        );

        if (respPostMessage) {
          console.log(respPostMessage);
          client?.emit("send-message-thread", respPostMessage?.data);
          pushThreadMessage(respPostMessage?.data);

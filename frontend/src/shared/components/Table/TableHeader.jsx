       ...
       <>
         <Box display="flex" justifyContent="space-between" alignItems="start">
           ...
           {headCell.hasSortIcon && (
             <IconButton onClick={() => toggleSort(headCell.id)} sx={{ padding: 0 }}>
               {sort[headCell.id] === ORDER_DIRECTION.asc ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
             </IconButton>
           )}
         </Box>
         <Box>{children}</Box>
...
export  const EnhancedTableHeader = ({
     ...
     return (
          ...
          {headCells.map(headCell => {
               ...
               align="left"
               >
               <TableHeaderCellWithAction headCell={headCell} sort={sort} toggleSort={toggleSort}>
